...
export class EasyTouch extends Component
{
    ...
    @property(UITransform) bgTransform: UITransform = null!;
    ...
    private _moveMaxDistance: number = 0;
    start()
    {
        this.directionNode.active = false;
        this._moveMaxDistance = this.bgTransform.width / 2;
        setTimeout(() =>
        {
    ...
    public moveTouch(x: number, y: number)
    {
        ...
        console.log(this.bgTransform.width);
        if (dis > this._moveMaxDistance)
        {
            var per: number = this._moveMaxDistance / dis;
            x = per * x;
            y = per * y;
        ...
        if (this.isLeftTouch)
        {
            PlayerData.instance.updateEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_LEFT_X, x / this._moveMaxDistance);
            PlayerData.instance.updateEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_LEFT_Y, y / this._moveMaxDistance);
        }
        // 右手
        else
        {
            PlayerData.instance.updateEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_RIGHT_X, x / this._moveMaxDistance);
            PlayerData.instance.updateEasyTouchInfo(Constant.EASY_TOUCH.TOUCH_RIGHT_Y, y / this._moveMaxDistance);
        }
    }
    ...
    public moveTouchByXY(x: number, y: number)
    {
        ...
        console.log("moveTouchByXY", x, y);
        this._lastX = Util.lerp(this._lastX, x, 0.1);
        this._lastY = Util.lerp(this._lastY, y, 0.1);
        console.log("moveTouchByXY _lastX", this._lastX);
        console.log("moveTouchByXY _lastY", this._lastY);
        this._vec3_1.set(this._lastX * this._moveMaxDistance, this._lastY * this._moveMaxDistance, 0);
        this.centerCircleNode.setPosition(this._vec3_1);
